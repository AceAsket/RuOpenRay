...
Крупные frontend-разделы лежат отдельными файлами рядом с `app.js`: `dashboard-view.js`, `servers-view.js`, `routing-view.js`, `dns-view.js`, `diagnostics-view.js`, `settings-view.js`, `setup-view.js`, `login-view.js` и диалоги. Диагностика дополнительно разложена на `diagnostics-domain-view.js`, `diagnostics-traffic-view.js` и `diagnostics-observatory-view.js`. Общие форматтеры вынесены в `formatters.js`, начальное состояние — в `state.js`, синхронизация настроек конфигурации — в `config-state.js`. Предметная логика теперь разнесена по небольшим моделям: `routing-model.js` и `routing-dsl.js` отвечают за правила и DSL-импорт, `xray-config-model.js` — за чтение Xray-конфига, `server-model.js` — за proxy/outbound и балансировщики, `dns-model.js` — за DNS-адреса и ответы, `devices-model.js` — за DHCP/LAN-устройства, `firewall-model.js` — за preview перехвата и nftables payload, `setup-model.js` — за черновик и снимки мастера настройки, `runtime-controller.js` — за refresh loop, live-логи и фоновые сэмплы трафика. Пользовательские действия тоже постепенно вынесены из `app.js`: `config-actions.js` — проверка, применение и статистика Xray, `settings-actions.js` — вход, сервисные настройки и логирование, `profile-actions.js` — профили и резервная копия, `import-actions.js` — импорт серверов и подписок, `server-actions.js` — проверка, переключение и fallback proxy-серверов, `observatory-actions.js` — Observatory/Burst Observatory для проверок и балансировщиков, `routing-actions.js` — редактор правил, подборки и порядок маршрутизации, `route-balancer-actions.js` — создание и порядок групп серверов, `xray-draft-actions.js` — черновики сниффера, TCP Fast Open, FakeDNS и inbound-подготовки, `setup-actions.js` — запуск мастера, применение и откат, `devices-actions.js` — правила LAN-устройств, `dns-actions.js` — DNS-черновик, hosts и LAN DNS, `firewall-actions.js` — применение перехвата и переключатели политики, `sni-actions.js` — SNI-сканирование, `updates-actions.js` — обновление Xray-core, RuOpenRay UI и geodata. Если разделу нужны DOM-события, рядом лежит соответствующий `*-bindings.js`. `app.js` остается точкой сборки состояния, API-зависимостей и действий.
