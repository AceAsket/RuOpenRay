...
RuOpenRay может принимать внешних клиентов через Xray Reality/VLESS и native AmneziaWG. Настройки сохраняются отдельно от активного Xray JSON, проходят preflight и применяются только явными действиями.

Для AmneziaWG панель умеет:

- создавать серверную и клиентскую X25519-пары ключей;
- собирать `client.conf` с адресом, endpoint, DNS, keepalive и параметрами обфускации;
- поднимать интерфейсы отдельным OpenWrt init-сервисом;
- создавать изолированную UCI firewall-зону с закрытым доступом к роутеру и LAN по умолчанию;
- выдавать каждому Xray-клиенту и AWG peer полный либо ограниченный LAN по приватным CIDR/IP, портам и протоколу;
- отдельно разрешать peer доступ к самому роутеру и его DNS/53, не открывая остальные сервисы;
- отдельно подтверждать открытие UDP-порта в WAN;
- блокировать конфликтующие интерфейсы, порты, подсети роутера и пересекающиеся peer `AllowedIPs`.

Приватный ключ клиента не записывается в server-mode JSON: он доступен в текущей вкладке только до обновления страницы. Скачайте `client.conf` сразу после создания ключа.

Managed AWG runtime сейчас поддерживает прямой выход в WAN. Выбор Xray outbound сохраняется в черновике, но запуск runtime блокируется до появления отдельной policy-routing схемы.

Для совместимости с b4 AWG policy использует `fwmark 0x52000000`, который не попадает в автоматический диапазон marks b4. При обновлении RuOpenRay также удаляет прежнее правило `0x5200`. Диагностика показывает числовые routing tables b4, scope NFQUEUE и прямые конфликты marks.
...
RuOpenRay UI имеет HTTP API под префиксом `/api`. Оно используется самой панелью и может применяться внешними интеграциями для диагностики, мониторинга, проверки proxy, чтения статуса firewall/DNS и аккуратного переключения режимов совместимости с B4.
