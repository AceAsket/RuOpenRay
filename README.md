...
Frontend устроен модульно: `app.js` только собирает состояние, API, действия и экраны. Отдельные разделы панели лежат в `cmd/ruopenray-ui/web/`, общие модели и форматтеры — рядом с ними, обработчики пользовательских действий — в `*-actions.js`, а привязка DOM-событий — в `*-bindings.js`. Подробный список файлов лучше смотреть в самой директории, чтобы README не дублировал структуру проекта и не устаревал после каждого рефакторинга.
