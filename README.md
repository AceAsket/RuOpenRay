...
Крупные frontend-разделы лежат отдельными файлами рядом с `app.js`: `dashboard-view.js`, `servers-view.js`, `routing-view.js`, `dns-view.js`, `diagnostics-view.js`, `settings-view.js`, `setup-view.js` и диалоги. Общие форматтеры вынесены в `formatters.js`, начальное состояние — в `state.js`, синхронизация настроек конфигурации — в `config-state.js`. Для маршрутизации есть отдельная модель `routing-model.js`: она отвечает за разбор, подписи, группировку и статистику правил. Базовые чтения Xray-конфига вынесены в `xray-config-model.js`: inbounds/outbounds, sniffer/DNS-mode и подписи серверов. Если разделу нужна отдельная подготовка данных или обработчики DOM-событий, рядом лежат `*-model.js` и `*-bindings.js`. `app.js` остается точкой сборки состояния, API-зависимостей и действий.
