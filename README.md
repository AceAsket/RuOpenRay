...
Актуальная публичная версия: `v0.4.0`. Кандидат на следующий релиз: `v0.5.0-rc.1`.
...
- пошаговый мастер базовой настройки с выбором сценариев и LAN-устройств до применения;
...
- несколько профилей AmneziaWG, импорт `client.conf`, runtime-метрики, балансировка и failover;
- маршруты напрямую в выбранный AWG-профиль или через технический outbound `Xray -> AWG`;
- серверный режим для управляемых VLESS/Reality inbound и AWG peers с preview, экспортом клиентов и WAN firewall;
