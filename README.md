...
Крупные frontend-разделы лежат отдельными файлами рядом с `app.js`: `dashboard-view.js`, `servers-view.js`, `routing-view.js`, `dns-view.js`, `diagnostics-view.js`, `settings-view.js`, `setup-view.js` и диалоги. Общие форматтеры вынесены в `formatters.js`, начальное состояние — в `state.js`, синхронизация настроек конфигурации — в `config-state.js`. Предметная логика теперь разнесена по небольшим моделям: `routing-model.js` и `routing-dsl.js` отвечают за правила и DSL-импорт, `xray-config-model.js` — за чтение Xray-конфига, `server-model.js` — за proxy/outbound и балансировщики, `dns-model.js` — за DNS-адреса и ответы, `devices-model.js` — за DHCP/LAN-устройства, `firewall-model.js` — за preview перехвата и nftables payload, `setup-model.js` — за мастер настройки, `runtime-controller.js` — за refresh loop, live-логи и фоновые сэмплы трафика. Пользовательские действия тоже постепенно вынесены из `app.js`: `config-actions.js` — проверка, применение и статистика Xray, `settings-actions.js` — вход, сервисные настройки и логирование, `devices-actions.js` — правила LAN-устройств, `dns-actions.js` — DNS-черновик, hosts и LAN DNS, `firewall-actions.js` — применение перехвата и переключатели политики, `sni-actions.js` — SNI-сканирование, `updates-actions.js` — обновление Xray-core, RuOpenRay UI и geodata. Если разделу нужны DOM-события, рядом лежит соответствующий `*-bindings.js`. `app.js` остается точкой сборки состояния, API-зависимостей и действий.
