...
Крупные frontend-разделы лежат отдельными файлами рядом с `app.js`: `dashboard-view.js`, `servers-view.js`, `routing-view.js`, `dns-view.js`, `diagnostics-view.js`, `settings-view.js`, `setup-view.js` и диалоги. Если разделу нужна отдельная подготовка данных или обработчики DOM-событий, рядом лежат `*-model.js` и `*-bindings.js`. `app.js` пока остается точкой сборки состояния и обработчиков.
