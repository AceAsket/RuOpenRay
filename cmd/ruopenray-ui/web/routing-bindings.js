export function bindRoutingControls({
  ...
  document.querySelectorAll('[data-route-values-panel]').forEach((button) => {
    button.addEventListener('click', (event) => {
      ...
      const left = fitsRight
        ? clickX + gap
        : Math.max(margin, clickX - width - gap);
      const top = Math.min(
  ...
  const decorateRouteTargetSelect = (select, encodedValue) => {
    ...
    triggerStatus.className = `route-target-status route-target-trigger-status ${selectedStatusClass}`;
    triggerStatus.title = selectedStatus?.label || '';
    triggerStatus.hidden = false;
    triggerStatus.classList.toggle('is-empty', !selectedStatusClass);
    ...
    trigger.title = selectedStatus?.label || triggerText.textContent || selectedOption?.value || '';
    ...
    [...select.options].forEach((option) => {
      ...
      optionStatusNode.className = `route-target-status route-target-option-status ${optionStatusClass}`;
      optionStatusNode.title = optionStatus?.label || '';
      optionStatusNode.hidden = false;
      optionStatusNode.classList.toggle('is-empty', !optionStatusClass);
      const optionText = document.createElement('span');
      optionText.className = 'route-target-option-text';
      optionText.textContent = routeTargetOptionLabel(option) || option.value;
      optionButton.title = optionStatus?.label || optionText.textContent;
